...
export function registerReaderTabPanel() {
  Zotero.ItemPaneManager.registerSections({
    ...
    head: {
      l10nID: "reader-use-dark-mode-for-content",
...
function onInit(options: any) {
  const body = options.body;
  const paneUID = Zotero_Tabs.selectedID;
  options.body.dataset.paneUid = paneUID;
  addon.data.panel.activePanels[paneUID] = options.refresh;
              ...
              listeners: [
                {
                  ...
                  listener: (e: Event) => {
                    ...
                    const itemID = options.getData().item?.id;
                    itemID &&
