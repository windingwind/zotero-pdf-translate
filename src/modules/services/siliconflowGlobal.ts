...
const SILICONFLOW_GLOBAL_PREF_PREFIX = "siliconflowGlobal";
...
const siliconflowGlobalTranslate = async function (
  data: Parameters<TranslateService["translate"]>[0],
) {
  const apiURL =
    (getPref(`${SILICONFLOW_GLOBAL_PREF_PREFIX}.endPoint`) as string) ||
    "https://api.siliconflow.com/v1/chat/completions";
  const model =
    (getPref(`${SILICONFLOW_GLOBAL_PREF_PREFIX}.model`) as string) ||
    "tencent/Hunyuan-MT-7B";
  const temperature = parseFloat(
    getPref(`${SILICONFLOW_GLOBAL_PREF_PREFIX}.temperature`) as string,
  );
  const stream = getPref(`${SILICONFLOW_GLOBAL_PREF_PREFIX}.stream`) as boolean;
  ...
  function transformContent(
    ...
    return transformPromptWithContext(
      `${SILICONFLOW_GLOBAL_PREF_PREFIX}.prompt`,
      langFrom,
      langTo,
      sourceText,
      data,
...
export const SiliconFlowGlobal: TranslateService = {
  id: "siliconflowGlobal",
  type: "sentence",
  helpUrl: "https://docs.siliconflow.ai/",
  ...
  async translate(data) {
    return await siliconflowGlobalTranslate(data);
  },

  config(settings) {
    settings
      .addTextSetting({
        prefKey: `${SILICONFLOW_GLOBAL_PREF_PREFIX}.endPoint`,
        nameKey: `service-siliconflowGlobal-dialog-endPoint`,
      })
      .addTextSetting({
        prefKey: `${SILICONFLOW_GLOBAL_PREF_PREFIX}.model`,
        nameKey: `service-siliconflowGlobal-dialog-model`,
      })
      .addNumberSetting({
        prefKey: `${SILICONFLOW_GLOBAL_PREF_PREFIX}.temperature`,
        nameKey: `service-siliconflowGlobal-dialog-temperature`,
        min: 0,
        max: 2,
        step: 0.1,
      })
      .addTextAreaSetting({
        prefKey: `${SILICONFLOW_GLOBAL_PREF_PREFIX}.prompt`,
        nameKey: `service-siliconflowGlobal-dialog-prompt`,
        placeholder: getString(`service-siliconflowGlobal-dialog-prompt`),
      })
      .addCheckboxSetting({
        prefKey: `${SILICONFLOW_GLOBAL_PREF_PREFIX}.stream`,
        nameKey: `service-siliconflowGlobal-dialog-stream`,
      });
  },
};
