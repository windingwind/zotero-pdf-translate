...
### Example
You are using Japanese, so search for *Japanese*. You get:  
```json
{"LangCultureName": "ja-JP", "DisplayName": "Japanese - Japan"},
```
The corresponding `"LangCultureName"` is `ja-JP`.  

Now we find the *Language Culture Code* `ja-JP`

### Language Culture Code Table
